...
#define ALT_VIBE_K_ALT_M_NSE    0.2f // increase coeff
...
void NavEKF3::UpdateFilter(void)
{
    ...
    // 
    _baroAltNoise_corr = constrain_ftype(_baroAltNoise * factor, ftype(0.1f), ftype(4.0f));
    _hgtInnovGate_corr = constrain_ftype(_hgtInnovGate * factor1, ftype(100.0f), ftype(1000.0f));

    static uint32_t _banomsg_last_ms = 0;
    const uint32_t now = AP_HAL::millis();
    if (now - _banomsg_last_ms >= 500) { //  Гц, чтобы не засорять лог
    _banomsg_last_ms = now;
    AP::logger().Write_MessageF("BANO Noise_corr=%.2f", (double)_baroAltNoise_corr);
    AP::logger().Write_MessageF("BANO I_gate_corr=%.2f", (double)_hgtInnovGate_corr);
    if (now - _banomsg_last_ms >= 250) {
    AP::logger().Write_MessageF("BANO Vibe_avr=%.2f", (double)vibe_avg);
    }
    }
    //
